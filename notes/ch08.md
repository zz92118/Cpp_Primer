...
- fstream和streamstring都是集成iostream  
## 前面章节已经在用的IO库设施

istream是输入流类型，cin是对象
...
### 管理输出缓冲（理解为打印buffer中的所有数据，并清空buffer）
...
- **文件流**：需要读写文件时，必须定义自己的文件流对象，并绑定在需要的文件上。
  - 可以使用fstream代替iostream& （因为fstream是继承了iostream类型的对象）
...
- 向string写入数据，从string读取数据。适用于对整行文本进行处理，而其他的一些工作是处理行内的单个单词。
- ostringstream 逐步构造输出（在此过程中修改），最后一起打印
