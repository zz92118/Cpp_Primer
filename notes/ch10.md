...
- 泛型算法本身不执行容器操作，只是单独依赖迭代器和迭代器操作实现。(不改变容器大小)
...
- 不依赖于容器所保存的元素类型，只要提供迭代器就可以（包括内置的数组和自己实现的数组arr[]）
...
string num = accumulate(v.cbegin(), v.cend(), "");//会产生编译错误，因为""是const char*类型，没有+运算符。
```
...
- **值捕获**：前提是变量可以拷贝，`size_t v1 = 42; auto f = [v1] {return v1;};`。被补货的变量的值实在lambda创建时拷贝，而不是调用时拷贝。
...
- 可变lambda：希望能改变一个背部获得变量的值。可以省略参数列表。
- **指定lambda返回类型**：使用尾置类型指定返回类型
...
- 函数如何包装成一元谓词？使用参数绑定。（可以完成类型lambda的功能）
- 标准库`bind`函数：
  ...
  - `auto g = bind(f, a, b, _2, c, _1);`，调用`g(_1, _2)`实际上调用`f(a, b, _2, c, _1)` 。**g到f的映射**
  - 使用bind重新排列参数
    ```cpp
...
- 从一个普通迭代器初始化一个反向迭代器，或是给一个反向迭代器赋值，结果迭代器和原迭代器指向的不是相同元素（左闭右开的区间）

```cpp
sort(vec.begin(),vec.end());// 正常排序
sort(vec.rbegin(),vec.rend());// 按照递减序列排序
```
...
- 一个高层迭代器支持低层迭代器类别的所有操作，向算法传递一个能力更差的迭代器会产生错误。
