...
- C++的所有容器
  - 顺序容器 vector，list，array，deque，string，forward_list
  - 关联容器
  - 容器适配器 stack，queue，priority_queue
- 遍历方式
  - `auto i:list` 返回的顺序容器中的元素对象
  - `for (auto iter = input.cbegin(); iter != input.cend(); ++iter)` 返回的是迭代器
...
- 新标准库的容器比旧版的快得多。
- vector和string在中间插入或者删除会移动之后所有的元素
- 占主导地位的操作类型（插入删除or执行访问）

## 容器操作

### 类型

- 迭代器范围 [begin,end) , end指向尾元素后一个的位置。
...
- 为了索引`int`的`vector`中的元素，应该使用什么类型？ `vector<int>::size_type`类型

- 为了读取`string`的`list`中的元素，应该使用什么类型？`list<string>::const_iterator // 读`
- `如果写入`list`，又应该使用什么类型？` `list<string>::iterator // 写`
- `list<string> a` 使用`a.rbegin()`获得`reverse_iterator`
...
| `reverse_iterator` | 反向迭代器，执行++操作得到上一个元素 |
...
|-----|-----|
| `C c;` | 默认构造函数，构造空容器 |
| `C c1(c2);`或`C c1 = c2;` | 构造`c2`的拷贝`c1` ，c1，c2必须是相同类型|
| `C c(b, e)` | 构造`c`，将迭代器`b`和`e`指定范围内的所有元素拷贝到`c`，array不适用 |
| `C c(a, b, c...)` | 列表初始化`c` |
| `C c(n)` | 只支持顺序容器，且不包括`array`，包含`n`个元素，这些元素进行了值初始化，且值都是explict的 |
...
- `C c(n)`  `list<string> svec(10,"hi")` 
- 如果元素类型没有默认构造函数，除了大小参数之外 一定要指定一个显示的初始值

-  接受一个容器创建其拷贝的构造函数，必须容器类型和元素类型都相同。
- 接受两个迭代器创建拷贝的构造函数，只需要元素的类型能够相互转换，容器类型和元素类型可以不同。

> 例子 使用迭代器创建的构造函数，只需要元素类型互相转换就可以
...
|assign（是替换的操作） 操作不适用于关联容器和array，导致左边容器内部的迭代器、引用、指针失效|
...
- 使用非成员版本的`swap`是一个好习惯。（泛型编程）
- 除了string，指向容器的迭代器、引用和指针在swap操作后都不会失效
...
- `emplace`开头的函数是新标准引入的，这些操作是构造而不是拷贝元素，在容器管理的内存中直接构造元素
- 传递给`emplace`的参数必须和元素类型的构造函数相匹配。（会调用元素的构造函数）
- 使用迭代器的方法，在顺序容器的指定位置插入元素`一定是迭代器p之前`
- 使用insert的返回值，在容器某个特定位置反复插入元素
...
- 访问成员函数返回的是引用。**可以用来改变元素的值**
  ```cpp
    if(!c.empty())
    {
      ...
      auto &v = c.back();// 是引用
      v = 1024;
      auto v2 = c.back();//不是引用 ，因为没有声明引用类型
    }
  ```
- `at`和下标操作只适用于`string`、`vector`、`deque`、`array`。
- `back`不适用于`forward_list`。
- 如果希望下标是合法的，可以使用`at`函数。（会抛出异常）

### 删除元素

| 操作 | 解释 |
|-----|-----|
| `c.pop_back()` | 删除`c`中尾元素，若`c`为空，则函数行为未定义。函数返回`void` (注意不是尾迭代器！！)|
| `c.pop_front()` | 删除`c`中首元素，若`c`为空，则函数行为未定义。函数返回`void` |
| `c.erase(p)` | 删除迭代器`p`指向的元素，返回一个指向被删除元素之后的元素的迭代器，若`p`本身是尾后迭代器，则函数行为未定义 |
| `c.erase(b, e)` | 删除迭代器`b`和`e`范围内的元素，返回指向最后一个被删元素之后元素的迭代器（尾后性质），若`e`本身就是尾后迭代器，则返回尾后迭代器 |
| `c.clear()` | 删除`c`中所有元素，返回`void` |

- 注意是`c.erase(p)`，中间的参数是迭代器类型
...
### 特殊的forwad_list操作（有特殊版本的删除和添加操作）
...
resize操作，元素类型必须提供一个默认构造函数。（如果自定义的类有任何一个构造函数，则默认构造函数将会失效）
...
### 容器操作可能使迭代器失效（比较难理解，再循环中删除或添加元素之和，需要重新计算迭代器）

**99也，3.4节的tips：凡是使用了迭代器的循环体，都不要向迭代器所属的容器添加元素（除非重新计算迭代器）**
...
- 在从一个容器中删除元素后：
  ...
  - 建议：不要保存`end`返回的迭代器。必须在每次插入操作后重新调用end()。
  - **每次改变容器的操作之后，需要重新定位迭代器！！！**
...
- `shrink_to_fit`只适用于`vector`、`string`和`deque`，只是一个请求，并不保证退还内存。
- `capacity`和`reverse`只适用于`vector`和`string`。
- `resize`函数只改变容器中元素的数目，不改变容量。
- 只要size()不超过capacity()，就不会为vector分配新的空间。
- `reverse` 如果预先知道了大概要用多少空间，可以用`reverse`预先分配一定的内存空间。
...
auto replace_with(string &s, string const& oldVal, string const& newVal)
{
    for (auto cur = s.begin(); cur <= s.end() - oldVal.size(); )
        if (oldVal == string{ cur, cur + oldVal.size() }) // 使用迭代器构造string
            cur = s.erase(cur, cur + oldVal.size()),
            cur = s.insert(cur, newVal.begin(), newVal.end()),//使用迭代器insert
            cur += newVal.size();
        else  
            ++cur;
}
...
- 创建适配器时，指定一个顺序容器，可以覆盖默认的基础容器：`stack<string, vector<string> > str_stk;` ， `stack<string, vector<string> > str_stk(svec);`。
****
- **牢记适配器是基于底层容器基础上的，但适配器可以有自己的操作。** leetcode有些题目，可以基于双端队列 自己实现stack或者queue。
****
### 适配器的通用操作和类型
